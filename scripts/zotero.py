...
return items.slice(0, {int(limit)}).map(i => ({
  ...
  creators: i.getCreators().map(c => c.ref ? c.ref.lastName : "").filter(Boolean).join(", "),
...
for (const id of itemIDs.slice(0, {int(limit)})) {{
  ...
  items.push({{
    ...
    creators: item.getCreators().map(c => c.ref ? c.ref.lastName : "").filter(Boolean).join(", "),
...
def create_item(meta):
    payload = {
        "itemType": meta.get("itemType", "journalArticle"),
    ...
    payload.update(meta.get("extra_fields", {}))
